...
import { loadSearchRoutes } from "@/features/public/route-persistence/load"
...
export default async function SearchPage({ searchParams }: PageProps<"/search">) {
  await searchParams
  const routes = await loadSearchRoutes()
  ...
  return <div className="marketplace-theme min-h-full bg-background text-foreground"><PageContainer className="py-8 sm:py-10"><SearchResults today={today} routes={routes} demo={!hasSupabaseEnvironment()} /></PageContainer></div>
}
