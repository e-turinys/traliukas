      ...
      <CardContent className="p-0">
        ...
        {active ? <form data-message-composer onSubmit={send} className="sticky bottom-0 z-10 rounded-b-xl border-t bg-card p-4 sm:p-6">
          ...
          <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-end">
            <Textarea id="message-body" value={body} onChange={event => { setBody(event.target.value); setSentNotice("") }} placeholder="Rašyti žinutę" rows={2} maxLength={2000} className="min-h-12 min-w-0 flex-1 resize-y bg-card" />
            <Button type="submit" disabled={!body.trim()} className="h-auto min-h-11 w-full shrink-0 whitespace-normal py-3 sm:w-auto">Siųsti<Send aria-hidden="true" className="size-4" /></Button>
          </div>
