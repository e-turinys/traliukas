...
import { dateLabel, readFilters, readSearch, requestHref } from "./search-query"
...
function MapPanel() {
  return <section aria-label="Maršrutų žemėlapis" className="flex min-h-[28rem] items-center justify-center rounded-xl border bg-muted/40 p-8 lg:sticky lg:top-6 lg:min-h-[36rem]">
...
function RouteList({ matches, servedSegment }: { matches: RouteMatch[]; servedSegment?: { from: LocationOption; to: LocationOption } }) {
  ...
  return <div className="space-y-4">
    <ul className="space-y-4">{matches.slice(0, limit).map((match) => <li key={match.route.id}><RouteCard {...match} servedSegment={servedSegment} /></li>)}</ul>
    {matches.length > limit && <Button variant="outline" className="min-h-11 w-full" onClick={() => setLimit((value) => value + 4)}>Rodyti daugiau</Button>}
  </div>
}
  ...
  return <div className="space-y-6">
    ...
    <Card><CardContent className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="min-w-0 space-y-1">
          ...
          <p className="text-sm text-muted-foreground">{search.invalid ? "Patikslinkite paiešką" : dateLabel(search.date)}</p>
        </div>
        <Button variant="outline" className="min-h-11" aria-expanded={showEditor} aria-controls="search-editor" onClick={() => setEditing(!editing)} disabled={search.invalid}>{showEditor ? copy.hide : copy.edit}</Button>
      </div>
      {showEditor && <div id="search-editor" className="space-y-4 border-t pt-5">
    ...
    {!search.invalid && <>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <Button variant="outline" className="hidden min-h-11 lg:inline-flex" aria-expanded={desktopFilters} aria-controls="desktop-search-filters" onClick={() => setDesktopFilters(!desktopFilters)}><SlidersHorizontal aria-hidden="true" />{copy.filters}{activeCount > 0 && ` (${activeCount})`}</Button>
          <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
            <SheetTrigger render={<Button variant="outline" className="min-h-11 w-full lg:hidden" />}><SlidersHorizontal aria-hidden="true" />{copy.filters}{activeCount > 0 && ` (${activeCount})`}</SheetTrigger>
            <SheetContent side="bottom" showCloseButton={false} className="max-h-[90dvh] overflow-y-auto">
      ...
      {desktopFilters && <div id="desktop-search-filters" className="hidden rounded-xl border p-5 lg:block"><SearchFilters key={query} query={query} apply={update} /></div>}
      <p className="text-sm text-muted-foreground">{copy.recommendation}</p>
      <p role="status" className="text-sm">Tinkami maršrutai: <strong>{exact.length}</strong>{alternatives.length > 0 && <> · Alternatyvūs maršrutai: <strong>{alternatives.length}</strong></>}</p>
      <div role="group" aria-label="Rezultatų rodymas" className="grid grid-cols-2 gap-2 lg:hidden">
        <Button variant={view === "list" ? "secondary" : "outline"} className="min-h-11" aria-pressed={view === "list"} aria-controls="search-route-list" onClick={() => setView("list")}><List aria-hidden="true" />Sąrašas</Button>
        <Button variant={view === "map" ? "secondary" : "outline"} className="min-h-11" aria-pressed={view === "map"} aria-controls="search-map" onClick={() => setView("map")}><Map aria-hidden="true" />Žemėlapis</Button>
      </div>
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,45fr)_minmax(0,55fr)]">
        <div id="search-route-list" className={cn("min-w-0 space-y-6", view === "map" && "hidden lg:block")}>
          ...
          {alternatives.length > 0 && <section className="space-y-4 border-t pt-6" aria-labelledby="alternative-heading">
            <h2 id="alternative-heading" className="text-lg font-semibold">Galimi alternatyvūs maršrutai</h2>
